...
const STATUSES = [
  ...
  { value: 'AET', label: 'After Extra Time' },
  { value: 'PEN', label: 'Penalty Shootout' },
  { value: 'CANC', label: 'Cancelled' },
]

interface Props {
  countries: Country[]
  initial?: Partial<MatchSummary>
  fixtureId?: number   // 수정 시 전달
}
...
export default function MatchForm({ countries, initial, fixtureId }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const body = {
      ...
      statusShort,
      statusLong: STATUSES.find((s) => s.value === statusShort)?.label ?? statusShort,
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <select
            ...
            className="w-full px-3 py-2.5 rounded-lg border border-border bg-background text-sm focus:outline-none focus:border-primary/60"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select
            value={statusShort}
            onChange={(e) => setStatusShort(e.target.value)}
            className="w-full px-3 py-2.5 rounded-lg border border-border bg-background text-sm focus:outline-none focus:border-primary/60"
          >
            {STATUSES.map((s) => (
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Teams &amp; Score</span>
        <div className="flex-1 h-px bg-border" />
      </div>

      {/* 홈팀 */}
      <div className="rounded-xl border border-border bg-card/50 p-4 space-y-3">
        <p className="text-xs font-bold text-primary uppercase tracking-wider">Home Team</p>
        <CountrySearchInput
          label="국가 검색"
          value={homeName}
          logoUrl={homeLogo}
        ...
        <div>
          ...
          <input
            ...
            placeholder="—"
            className="w-24 px-3 py-2 rounded-lg border border-border bg-background text-sm focus:outline-none focus:border-primary/60"
          />
        </div>
      </div>
      ...
      {/* 어웨이팀 */}
      <div className="rounded-xl border border-border bg-card/50 p-4 space-y-3">
        <p className="text-xs font-bold text-primary uppercase tracking-wider">Away Team</p>
        <CountrySearchInput
          label="국가 검색"
          value={awayName}
          logoUrl={awayLogo}
