    ...
    <div ref={ref} className="relative">
      ...
      <div className="flex items-center gap-2 px-3 py-2.5 rounded-lg border border-border bg-background focus-within:border-primary/60 transition-colors">
        {logoUrl && (
          ...
          <img src={logoUrl} alt="" className="w-6 h-4 object-cover rounded-sm flex-shrink-0" />
        )}
        <input
          type="text"
          value={query}
        ...
        <div className="absolute z-50 mt-1 w-full max-h-56 overflow-y-auto rounded-lg border border-border bg-card shadow-xl">
          {filtered.map((c) => (
            ...
            >
              {c.flagUrl ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={c.flagUrl} alt="" className="w-7 h-5 object-cover rounded-sm flex-shrink-0" />
              ) : (
              ...
              <div>
                ...
                {c.groupName && (
                  <p className="text-[10px] text-muted-foreground">Group {c.groupName}</p>
                )}
              </div>
            </button>
          ))}
